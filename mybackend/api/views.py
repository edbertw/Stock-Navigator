...
def submit_stock(request):
    ...
    if company:
        ...
        try:
            stock = yf.download(stock_symbol, start = start, end = end, progress = False)
            if stock.empty:
            ...
            stock_data['Upper Band'] = stock_data['20-Day MA'] + (stock_data['Close'].rolling(window=20).std() * 2)
            stock_data['Lower Band'] = stock_data['20-Day MA'] - (stock_data['Close'].rolling(window=20).std() * 2)

            # Create Bollinger Bands chart
            fig_bollinger = go.Figure()
            fig_bollinger.add_trace(go.Scatter(x=stock_data['Date'], y=stock_data['Close'], name='Stock Price'))
            fig_bollinger.add_trace(go.Scatter(x=stock_data['Date'], y=stock_data['Upper Band'], name='Upper Band', fill=None, mode='lines'))
            fig_bollinger.add_trace(go.Scatter(x=stock_data['Date'], y=stock_data['Lower Band'], name='Lower Band', fill='tonexty', mode='lines'))
